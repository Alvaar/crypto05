...
int main(int argc, char* argv[])
{
	...
	/*printf("  Генерация простых чисел P и Q (%i bits): \n", size_b);
	mpi_gen_prime(&P, size_b, 0, myrand, NULL);
	mpi_gen_prime(&Q, size_b, 0, myrand, NULL);
	mpi_write_file("  P = ", &P, 16, NULL);
	mpi_write_file("  Q = ", &Q, 16, NULL);
	...
	printf("  Генерация числа E: \n");
	bool error = true; mpi_init(&G);
	while (error)
	{
		mpi_gen_prime(&E, size_b, 0, myrand, NULL);
		mpi_gcd(&G, &E, &H);
		if (mpi_cmp_abs(&G, &E) == -1)
			error = false;
	}
	...
	int msg_len = msg.length(); //printf("%s", msg_in);
	...
	unsigned char* msg_ver = new unsigned char[msg_len];
	ex = mpi_write_binary(&X, (unsigned char*)msg_ver, msg_len);
	
	unsigned long block;
	GET_ULONG_BE(block, msg_ver, 0);
	if (block % 2 == 0)
		block++;
	else
		block--;
	PUT_ULONG_BE(block, msg_ver, 0);

	mpi_read_binary(&S, msg_ver, msg_len);
	mpi_write_file("           измененный текст: ", &S, 16, NULL);
	printf("           измененный текст: %.*s \n", msg_len, msg_ver);
	ex = rsa_ver(&MM, &S, &E, &N);
